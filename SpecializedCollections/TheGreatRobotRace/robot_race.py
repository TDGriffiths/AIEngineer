...
while not rr.is_race_over(bots) and num_of_turns < max_turns:
  ...
  for bot in bots:
    if not bot.has_finished:
      ...
      print(rr.compute_robot_logic(walls, goal, bot))
...
# Add your code below!
print(robot_moves)
move_counter = Counter(move[0] for move in robot_moves)


# Count the number of collisions by robot name
# Add your code below!
collision_counter = Counter(move[0] for move in robot_moves if move[2] == True)
...
# Add your code below!
for bot in bots:
  bot_scores.append(BotScoreData(bot.name, move_counter[bot.name], collision_counter[bot.name], move_counter[bot.name] + collision_counter[bot.name]))
print(bot_scores)
